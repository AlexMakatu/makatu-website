...
export default function FloatingCTA({
  ...
  useEffect(() => {
    const handleScroll = () => {
      setVisible(window.scrollY > 200); // show after scroll
    };
  ...
  return (
    <>
      ...
      >
        ...
        <Link
          href={quoteUrl}
          className="bg-[#311d60] text-white px-7 py-4 rounded-xl shadow-2xl font-semibold hover:scale-105 hover:shadow-2xl transition text-base"
        >
          Get a Quote
        </Link>
        ...
        <a
          href={whatsappUrl}
          target="_blank"
          className="text-sm text-gray-700 bg-white px-4 py-2 rounded-lg shadow hover:bg-gray-50 transition"
        >
          💬 Talk to an agent
        </a>
      </div>
