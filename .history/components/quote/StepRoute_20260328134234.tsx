...
export default function StepRoute({ next, data }: Props) {
  ...
  async function handleAI() {
    ...
    try {
      ...
      if (parsed.fromCity) setFromCity(parsed.fromCity);
      if (parsed.toCity) setToCity(parsed.toCity);

      // auto continue if complete
      if (parsed.fromCity && parsed.toCity) {
        next({
    ...
    <form onSubmit={submit} className="space-y-6">
      ...
      <div className="space-y-3">
        ...
        <button
          type="button"
          onClick={handleAI}
          className="w-full py-3 rounded-xl text-sm font-medium bg-[#311d60] text-white"
        >
          {loadingAI ? "Processing..." : "Quick Estimate ⚡"}
        </button>
