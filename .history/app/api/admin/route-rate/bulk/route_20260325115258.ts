...
import { writeClient } from "@/sanity/lib/client"; // ✅ IMPORTANT FIX
  ...
  try {
    ...
    for (const row of parsed) {
      try {
        ...
        let price = row.price ? Number(row.price) : undefined;
        let priceType = row.priceType;

        // 🔥 AUTO RULE: if price is 0 or missing → quoteRequired
        if (!price || price === 0) {
        ...
        const rateId = `routeRate-${from._id}-${to._id}-${row.vehicleType}`;
