...
async function sendToZoho(data: QuoteRequestBody) {
  ...
  // ✅ SAFE DEFAULTS (only when blank)
  const safeTransportType = data.transportType || "doorToDoor";
  const safeCollectionType = data.collectionDateType || "flexible";
  const safeCustomerType = data.customerType || "private";
  ...
  const payload = {
    data: {
      ...
      // ✅ always valid (with defaults)
      Customer_Type: mapCustomerType(safeCustomerType),

      From_City: data.fromCity,
      To_City: data.toCity,

      Transport_Type: mapTransportType(safeTransportType),
      Collection_Date_Type: mapCollectionType(safeCollectionType),

      // ✅ ONLY include if specific date
      ...(safeCollectionType === "specificDate" &&
        data.collectionDate && {
