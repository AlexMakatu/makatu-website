      ...
      validation: (Rule) =>
        Rule.required().custom((sections: unknown) => {
          ...
          safeSections.forEach((section, index) => {
            ...
            if (s.questions) {
              s.questions.forEach((q) => {
                sectionText += " " + q.question + " " + q.answer;
              });
            }

            const count = sectionText.split(/\s+/).filter(Boolean).length;

            console.log(
              `SECTION ${index + 1} (${section._type}${
                "sectionKey" in section ? `:${section.sectionKey}` : ""
              }) → ${count} words`,
            );
          });
          ...
          if (cost) {
            ...
            const routes = (
              text.match(
                /(Johannesburg|Cape Town|Durban|Pretoria|Bloemfontein|Gqeberha)\s+to\s+(Johannesburg|Cape Town|Durban|Pretoria|Bloemfontein|Gqeberha)/gi,
              ) || []
            ).length;
